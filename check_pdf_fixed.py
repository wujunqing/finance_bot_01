...
def move_invalid_files(invalid_files, backup_dir):
    """将无效文件移动到备份目录"""
    ...
    for file_path, error in invalid_files:
        ...
        try:
            ...
            print(f"已移动: {filename} -> {backup_dir}")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    pdf_dir = os.path.join(current_dir, "dataset", "pdf")
    backup_dir = os.path.join(current_dir, "dataset", "pdf_backup")
    
    print(f"当前工作目录: {os.getcwd()}")
    print(f"PDF目录: {pdf_dir}")
    print(f"目录是否存在: {os.path.exists(pdf_dir)}")
    ...
    if invalid_files:
        ...
        if response.lower() == 'y':
            move_invalid_files(invalid_files, backup_dir)
